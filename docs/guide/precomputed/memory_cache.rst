...
earthkit-geo provides an **in-memory cache** for interpolation pre-computed interpolation weights. When it is enabled, weights loaded from the disk are stored in memory and when we call :func:`regrid` with the same grids they do not have to be loaded from disk again. The cache can be configured to have a maximum size and eviction policy.

.. note::

    Please note that the earthkit-geo in-memory cache configuration is managed through the :doc:`config`.
...
.. code:: python
  ...
  >>> from earthkit.geo import memory_cache_info
  >>> memory_cache_info()
...
.. code:: python
  ...
  >>> from earthkit.geo import clear_memory_cache
  >>> clear_memory_cache()
  >>> memory_cache_info()
