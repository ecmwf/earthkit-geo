...
.. note::
    ...
    This caching is only related to the :ref:`precomputed <precomputed-backend>`
...
.. note::
    ...
    Please note that the earthkit-geo in-memory cache configuration is managed through the :ref:`config`.
...
The current status of the in-memory cache can be retrieved using the :func:`precomputed_memory_cache_info` function. It returns a namedtuple with fields ``hits``, ``misses``, ``maxsize``, ``currsize``, ``count`` and  ``policy``.
...
The in-memory cache can be cleared using the :func:`clear_precomputed_memory_cache` function.
...
.. warning::
  ...
  These config options are only used when ``regrid-precomputed-weights-memory-cache-policy`` is :ref:`largest <largest_mem_cache_policy>` or :ref:`lru <lru_mem_cache_policy>`.
...
.. module-output:: generate_config_rst regrid-precomputed-weights-memory-cache-policy regrid-precomputed-weights-maximum-memory-cache-size regrid-precomputed-weights-memory-cache-strict-mode
