...
OLD_CONFIG_FILE = os.path.expanduser("~/.earthkit_geo/settings.yaml")
CONFIG_DIR = os.path.expanduser("~/.config/earthkit/geo")
CONFIG_FILE_NAME = "config.yaml"
CONFIG_FILE_ENV_NAME = "CONFIG_FILE"
ENV_PREFIX = "EARTHKIT_GEO_"
...
def save_config(path, config):
    ...
    with FileLock(lock):
        with open(path, "w") as f:
            ...
            print(file=f)
            print("#", "-" * 76, file=f)
            print("# Version of earthkit-geo", file=f)
            print(file=f)
...
class Config:
    ...
    def _save(self):
        ...
        try:
            ...
            LOG.error(
                "Cannot save earthkit-geo config (%s)",
                self._config_yaml,
                exc_info=True,
            )
    ...
    def save_as(self, path):
        try:
            ...
            LOG.error(
                f"Cannot save earthkit-geo config ({path})",
                exc_info=True,
            )
...
def _migrate_old_config(config_file):
    ...
    if os.path.exists(old_config_file):
        ...
        try:
            ...
            LOG.error(
                f"Cannot migrate old settings to earthkit-geo config file={config_file}",
                exc_info=True,
            )
    return False
...
def _init_config_file():
    ...
    if config_file is None:
        LOG.error("Cannot construct earthkit-geo config file path")
        return None
    ...
    try:
        ...
        LOG.error(
            f"Cannot create earthkit-geo config directory={config_dir}",
            exc_info=True,
        )
    ...
    if os.path.exists(config_dir):
        try:
            ...
            LOG.error(
                f"Cannot save settings to earthkit-geo config file={config_file}",
                exc_info=True,
            )
...
def _init_config(config_yaml):
    ...
    try:
        ...
        LOG.error(
            "Cannot load earthkit-geo config (%s), reverting to defaults",
            config_yaml,
            exc_info=True,
        )
