...
class CacheManager(threading.Thread):
    ...
    def _housekeeping(self, clean=False):
        ...
        with self.connection as db:
            for name in os.listdir(top):
                ...
                if parent is None:
                    LOG.warning(f"earthkit-geo cache: orphan found: {full}")
                else:
    ...
    def _delete_entry(self, entry):
        ...
        # LOG.warning(f"earthkit-geo cache: deleting {path} ({humanize.bytes(size)})")
        # LOG.warning(f"earthkit-geo cache: {owner} {args}")
