...
from earthkit.geo.utils import humanize
...
class CacheManager(threading.Thread):
    ...
    def _housekeeping(self, clean=False):
        ...
        with self.connection as db:
            for name in os.listdir(top):
                ...
                if parent is None:
                    LOG.debug(f"earthkit-geo cache: orphan found: {full}")
                else:
                    LOG.debug(f"earthkit-geo cache: orphan found: {full} with parent {parent}")
    ...
    def _delete_entry(self, entry):
        ...
        LOG.debug(f"earthkit-geo cache: deleting {path} ({humanize.bytes(size)})")
        LOG.debug(f"earthkit-geo cache: {owner} {args}")
    ...
    def _decache(self, bytes, purge=False):
        ...
        LOG.warning("earthkit-geo cache: trying to free %s", humanize.bytes(bytes))
        ...
        with self.connection as db:
            ...
            for stmt in (
                ...
                for entry in db.execute(stmt, (latest,)):
                    ...
                    if total >= bytes:
                        LOG.warning(
                            "earthkit-geo cache: freed %s from cache",
                            humanize.bytes(bytes),
                        )
                        return total

        LOG.warning("earthkit-geo cache: could not free %s", humanize.bytes(bytes))
        ...
        if self._policy.managed():
            ...
            if maximum is not None:
                ...
                if size > maximum:
                    ...
                    LOG.warning(
                        (
                            f"earthkit-geo cache: cache size({humanize.bytes(size)}) exceeds limit"
            ...
            if usage is not None:
                ...
                if df.percent > usage:
                    ...
                    LOG.warning(
                        (
                            f"earthkit-geo cache: filesystem usage ({df.percent}%) exceeds limit ({usage}%)"
